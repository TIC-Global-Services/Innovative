      ...
      <div className="py-10 md:py-20 flex flex-col items-center space-y-10 md:space-y-16 px-4 md:px-0">
        ...
        <div className="w-full md:max-w-[80%] ">
          {items.map((item, index) => (
            <div
              key={index}
              className="border-b border-gray-300/50 pb-6 mb-6 md:mb-8"
            >
              <div
                className="flex flex-col sm:flex-row justify-between items-start gap-3 sm:gap-0 sm:items-center w-full cursor-pointer "
                onClick={() => toggleItem(index)}
              >
                <div className="flex items-start justify-between md:justify-start w-full sm:items-center gap-2 md:gap-9 max-w-full sm:max-w-full md:max-w-[90%] ">
                  ...
                  <h3 className="text-base text-center md:text-left sm:text-2xl font-medium text-[#040444]">
                    {item.question}
                  </h3>
                  <motion.div
                  ...
                  >
                    <Plus
                      size={openIndex === index ? 28 : 24}
                      strokeWidth={openIndex === index ? 2 : 2}
                      className="ml-auto"
                    />
                  </motion.div>
                </div>
